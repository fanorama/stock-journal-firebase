...
/**
 * Load trade data into form when trade prop changes
 */
watch(
  () => props.trade,
  (newTrade) => {
    if (newTrade) {
      formData.value = {
        ...
        date: newTrade.date.toDate ? newTrade.date.toDate() : newTrade.date,
...
const handleSubmit = async () => {
  if (!props.trade) {
    submitError.value = 'Trade tidak ditemukan'
    return
  }
        ...
        <div class="bg-[#fafafa] border-t-[5px] border-black p-6 sticky bottom-0">
          <div class="flex gap-4 justify-end">
            ...
            >
              ...
              {{ isSubmitting ? 'Menyimpan...' : 'Update Trade' }}
            </button>
          </div>
        </div>
      </div>
