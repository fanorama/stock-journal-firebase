...
/**
 * Watch linkedTradeId prop changes
 */
watch(
  () => props.linkedTradeId,
  (newTradeId) => {
    if (newTradeId) {
      formData.value.tradeId = newTradeId
    }
  }
)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const journalId = await journalsStore.createJournal(input)

    // Show success toast
    toast.success('Journal entry berhasil ditambahkan!')

    emit('success', journalId)
    emit('close')
    resetForm()
  } catch (err: any) {
    const errorMessage = err.message || 'Gagal membuat journal entry'
    error.value = errorMessage
    // Show error toast
    toast.error(`Gagal menambahkan journal entry: ${errorMessage}`)
  } finally {
  ...
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        ...
        >
          <!-- Modal Header -->
          <div class="bg-[#a855f7] border-b-[5px] border-black p-6 flex items-center justify-between">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-6">
              ...
              <div class="bg-[#fef3c7] border-[3px] border-black p-4">
                <p class="text-sm font-mono text-[#525252]">
                  📌 Document your trading thoughts, strategies, and lessons. At least one field must be filled.
                </p>
              </div>
          ...
          <div class="border-t-[5px] border-black p-6 bg-[#fafafa] flex gap-3 justify-end">
            ...
            <button
              type="submit"
              class="bg-[#a855f7] border-[3px] border-black px-6 py-3 font-bold uppercase text-sm tracking-wide text-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-100"
              @click="handleSubmit"
